...
//     const TypewriterText: React.FC<Typewriter> = ({ text, speed = 100, onComplete, showCursor = true, cursorAlwaysOn = false, erase = false, onEraseComplete, }) => {
...
export default function TypewriterText({
  ...
  // Scrittura
  useEffect(() => {
    if (erase) return; // Non scrivere se stiamo cancellando
  ...
  useEffect(() => {
    ...
    return () => clearInterval(interval);
  }, [erase, currentText, speed, onEraseComplete]);

  return (
    <span>
      {currentText}
      {(showCursor || cursorAlwaysOn) && <span className="animate-blink">|</span>}
    </span>
  );
}
